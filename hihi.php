      ...
      if (sasql_num_rows($result) > 0){
        while($row = sasql_fetch_object($result)){
          ...
          while($check_prev_day = sasql_fetch_object($result_prev)){
            ...
            if (!empty($next_in) && $check_prev_day->type == 1 && $check_prev_day->row_num != 1 && $check_prev_day->row_num != $check_prev_day->rows){
              echo $row->empid." - ".$first_out." - ".$next_in." - ".$check_prev_day->row_num."<br>";
              $hour_d = strtotime($next_in) - strtotime($first_out);
              ...
              echo $row->empid." - Last out(".$first_out.")<br>";
            }
          }
          ...
          if($row_count == 0){
            ...
            while($check_row = sasql_fetch_object($result_check)){
                      ...
                      if ($hour_diff >= '08:00:00'){
                        ...
                        if ($first_break == 0){
                          ...
                          if ($check_diff >= '08:00:00' && $log_row->type == 2 && $log_row->row_num == $log_row->rows && $time_out == 0){
                            ...
                            $time_out = 1;
                            if ($log_row->guard_remarks == ' ' || $log_row->guard_remarks == ''){
                              $string .= "<td name='s". $log_row->timevar."' value=".$log_row->tran_id." id='".$log_row->timestamp."' class='hidden_out_time collapse'></td>";
                              $string .= '<script type="text/javascript">
                                   out_time('.$time_out.');
                                   </script>';
                            } else {
                    ...
                    //echo $checker." ".$last_time." ".$check_next->row_num."<br>";
                      if($check_next->type == 2){
                        ...
                        if($hour_dif >= '08:00:00'){
                          if ($check_next->row_num == $check_next->rows){
                            if ($first_break == 0){
                              $time_out = 2;
              ...
              if ($shift_dur < '08:00:00'){
                ...
                $string .= "<td name='Undertime' class='undertime'>".$undertime."</td>";
      ...
      $(document).ready(function(){
         ...
         function update(){ 
            ...
            clicked = 0;
              ...
              if ($check_current_day->type == 2){
                $current_first_out = $check_current_day->timestamp;
              }

              if ($check_current_day->row_num != 1 && $check_current_day->type == 1){
                $current_in = $check_current_day->timestamp;
              }

              if (!empty($current_in) && $check_current_day->type == 1 && $check_current_day->row_num != 1 && $check_current_day->row_num != $check_current_day->rows){
                echo $row->empid." - ".$current_first_out." - ".$current_in." - ".$check_current_day->row_num."<br>";
                $current_hour = strtotime($current_in) - strtotime($current_first_out);
                $current_hour = sprintf('%02d:%02d:%02d', ($current_hour/ 3600),($current_hour/ 60 % 60), $current_hour% 60);
                //echo $hour."<br>";
              } else if ($check_current_day->type == 2 && $check_current_day->row_num == $check_current_day->rows){
                $current_last_out = $check_current_day->timestamp;
                echo $row->empid." - Last out(".$current_last_out.")<br>";
                $current_last = 1;
              } else if ($check_current_day->type == 1 && $check_current_day->row_num == $check_current_day->rows){
                if ($check_current_day->rows > 1){
                  echo $row->empid." - ".$current_first_out." - ".$current_in." - ".$check_current_day->row_num."<br>";
                  $current_hour = strtotime($current_in) - strtotime($current_first_out);
                  $current_hour = sprintf('%02d:%02d:%02d', ($current_hour/ 3600),($current_hour/ 60 % 60), $current_hour% 60);
                  echo $current_hour."(current DAY)<br>";
                } else {
                  //no out yet
                }
              }

              if ($current_hour >= '08:00:00'){
                if ($check_current_day->type == 1 && $current_last == 1){  
                  $start = $check_current_day->row_num - 1;
                } else {
                ...
                $last = 0;
                  ...
                  if ($check_current_day->type == 2){
                    $first_out = $check_current_day->timestamp;
                  }
                  ...
                  if (!empty($current_in) && $check_current_day->type == 1 && $check_current_day->row_num != 1 && $check_current_day->row_num != $check_current_day->rows){
                    //echo $row->empid." - ".$first_out." - ".$next_in." - ".$check_prev_day->row_num."<br>";
                    $current_hour = strtotime($current_in) - strtotime($first_out);
                    $current_hour = sprintf('%02d:%02d:%02d', ($current_hour/ 3600),($current_hour/ 60 % 60), $current_hour% 60);
                    //echo $hour."<br>";
                  } else if ($check_current_day->type == 2 && $check_current_day->row_num == $check_current_day->rows){
                    $current_last_out = $check_current_day->timestamp;
