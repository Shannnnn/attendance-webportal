...
<html>
  ...
  <body>
    ...
    <div class="col-md-4" id="date-div">        
      <h2 style="text-align: center;"><?php $today_date = date_create($date); echo date_format($today_date,"F d, Y"); ?></h2>
      <h6 id='emp_count' style="text-align: center;"></h6>
    </div>
      
    <div class="col-md-3" id="input-div">
      <input data-provide="datepicker" data-date-format="yyyy-mm-dd" id="date-input" placeholder="Date">
      <span>
        <button type="button" class="btn btn-dark" id="go-btn">Go</button>
            </span><br>
      
      <input type="text" id="name-input" placeholder="Search for names"/>
    </div>
    
    <div class="col-md-2" id="buttons-div">
      <button type="button" class="btn btn-primary" id="in-out-btn"><img src="img/door.png"/></span>In/Out</button><br>
      <button type="button" class="btn btn-success" id="add-emp-btn"><img src="img/add.png"/>Add Employee</button>
    </div>
    
    <a style="position: absolute; right: 0; margin-right: 2em; margin-top:1.4em; width:12em;" href="logout.php" class="btn btn-dark">Log Out</a>
    </div>
        ...
        while($row = sasql_fetch_object($result)){
              ...
              if ($once == 0 && $prev_in > $prev_out && $prev_in != strtotime('00:00:00') && $prev_out != strtotime('00:00:00')){
                ...
                if ($hourr >= '08:00:00' && $hourr != strtotime('00:00:00')){
                  ...
                  if ($counter == 2){
                    ...
                    $next_row = $check_prev_day->row_num;
          ...
          if (sasql_num_rows($result_current) != 0){
            while($check_current_day = sasql_fetch_object($result_current)){
              ...
              if ($last == 0){  
                if ($hour_n >= '07:00:00' && $hour_n != strtotime("00:00:00")){ //changed
                  ...
                  if (sasql_num_rows($result_prev) != 0){
                    //echo $has_out." - ".$check_current_day->row_num."<br>";
                    $start_time = 0;//$check_current_day->row_num; //Possible
                    $next_time = $start_time;
                    $last = 2;
                  } else {
                    $start_time = $check_current_day->row_num; //Possible
                    $next_time = $start_time;
                    $last = 2;
                  }
                }  
              } else if ($last == 1){ //added for $last
                  //echo 'capiche'; //do this today!!!!
                if ($hour_n >= '07:00:00' && $hour_n != strtotime("00:00:00")){
                  //echo $row->empid." - ".$hour_n."<br>"; 
                  $limit = $check_current_day->row_num;
                  $last = 1;
                }
              }

              if ($has_out == 1){
                //echo $row->empid." ".$check_current_day->timestamp."<br>"; 
              ...
              while($check_day_day = sasql_fetch_object($result_day)){
                  ...
                  /**/if ($day_last_out != strtotime("00:00:00") && $day_last_out > $day_first_in && $check_day_day->row_num == 2){
                    ...
                    if ($day_current_hour > '01:00:00'){
                      ...
                      if ($day_current_hour > '08:00:00'){
                        $out_count2++;
                        $last_limit2 = $check_day_day->row_num;
                      }
                    }
                  }
                  ...
                  if ($day_hour >= '08:00:00' && $day_hour != strtotime("00:00:00")){  
                    ...
                    //} else {
                      if ($out_count == 1){
                        ...
                        $last = 2; //addeddddd
                        //echo $row->empid." - ".$check_day_day->timestamp."<br>";
                        //echo $row->empid." - ".$start."<br>";
                      } else if ($out_count > 1 && $check_day_day->row_num == $check_day_day->rows) {
                        if ($out_count2 == 1){
                          $start = $check_day_day->rows - 2;
                          $last = 2; //addedddd+
                          //echo $row->empid." - ".$check_day_day->timestamp." 2<br>";
                          //echo $row->empid." - ".$start." two<br>";
                        } else {
                          $start = $last_limit2 + 1;
                          $last = 2; //addeddddd
                          //echo $row->empid." - ".$check_day_day->timestamp." 2<br>";
                    ...
                    //echo $row->empid." - ".$check_day_day->row_num."<br>";
                    //if ($has_prev == 0 && ($check_day_day->row_num - 1) == 1){
                    //  $start_time = $check_day_day->row_num - 1; //here
                    //  $next_time = $start_time;
                    //}
              ...
              while($check_next_day = sasql_fetch_object($result_third)){
                ...
                if ($check_next_day->type == 2 && $check_next_day->row_num == $check_next_day->rows){
                  ...
                  if ($next_hour >= '08:00:00' && $next_hour != strtotime("00:00:00")){
                    $limit = $check_next_day->rows;// - 1; changed
                    $last = 2;
                    //echo $limit."<br>";
                  } else {
          ...
          //added for prev
          //echo $row->empid." - ".$last."<br>";
          if ($last == 1){
            $next_day = $date;
            $date = date('Y-m-d', strtotime($date . ' -1 day'));
            $next_time = $next_row;
            //echo $row->empid." ".$next_time." - ".$start." ".$date."<br>";
            //$limit = $start;
            //echo $limit;
          }
          ...
          if($row_count == 0){
            ...
            while($check_row = sasql_fetch_object($result_check)){
              ...
              if ($first_type == 1){
                ...
                $emp_log = "select empid as id, tran_id, convert(varchar(5),timestamp, 101) + right(convert(varchar(32),timestamp, 100), 8) as timevar, timestamp, type, row_number() over (order by timestamp asc) row_num, count(*) over () as rows, guard_remarks
                              from dba.logs
                              where timestamp like '".$date."%' and empid = ".$row->empid." and active = 1
                              order by timestamp asc
              ...
              while($log_row = sasql_fetch_object($result_log)){
                //echo $row->empid." ".$log_row->timevar."<br>";
                if ($first_type == 1){
                  if ($log_row->type == 2){
                      ...
                      if ($hour_diff >= '08:00:00'){
                        ...
                        if ($first_break == 0){
                          ...
                          if ($check_diff >= '08:00:00' && $log_row->type == 2 && $log_row->row_num == $log_row->rows && $time_out == 0){
                            ...
                            //for mising time in
                            if ($log_row->row_num % 2 == 1 && $log_row->type == 2 && $log_row->row_num != 1){
                              $first_break = 2;
                              $break_count++;
                              $string .= "<td value='0' name='1' class='break_in' bgcolor='#FF0000'>Missing break in!</td>";
                    ...
                    //echo $checker." ".$last_time." ".$check_next->row_num."<br>";
                      if($check_next->type == 2){
                        ...
                        if ($first_break == 1){
                          ...
                          if ($check_next->row_num == 1){
              if ($next_time == 1 && $start == 1){ //check
              //if log->rows = 1 then it has red out and missing break out else only missing break out lang
                $first_break = 2;
